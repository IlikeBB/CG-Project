...
for train_index, test_index in skf.split(X, y):
    ...
    rnn = RNN(max(y)+1, Input_Size).to(device)
    if hasattr(torch.cuda, 'empty_cache'):
        torch.cuda.empty_cache()
    ...
    for epoch in tqdm(range(400)):
        ...
        with torch.no_grad():
            ...
            if min_valid_loss > valid_loss:
                ...
                # Saving State Dict
                torch.save({'state_dict': rnn.state_dict()}, f'./pth/LSTM_{week_}saved_model_{filter_ths}_{fold_n}.pth.tar')
    plt.figure(figsize=(10,10))
    plt.plot(tloss_stack, label='train loss')
    plt.plot(vloss_stack, label='valid loss')
    plt.xlabel('Epochs', fontsize=18)
    plt.ylabel('CE loss',fontsize=18)
    plt.title(f'LSTM {week_[::-1]} model Loss plot [nums > {filter_ths}]', fontsize=25)
    plt.legend()
    plt.savefig(f'./results/{filter_ths}_loss_LSTM_{week_}{fold_n}.jpg')
    plt.close(2)
