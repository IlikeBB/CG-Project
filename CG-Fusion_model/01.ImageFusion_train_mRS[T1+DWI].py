...
# load csv label
if True:
    csv_path = './csv/NIHSS_fiter_222patient(mRS).csv'
    dwi_dir =  './dataset/DWI_1-350/'
    t1_dir = './dataset/T1_1-350/'
    table_ =  pd.read_csv(csv_path, index_col=False)
    table_label = table_['出院mRS']
    temps_ = list(table_label)
    # print(table_label)
    table_label = [0 if i in [0,1,2] else 1 for i in table_label ]
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(stream, start=1):
        images_dwi = data['dwi'].to(device)
        images_T1 = data['t1'].to(device)
        # print(images_dwi.shape, images_T1.shape, i)
        target = torch.LongTensor(data['target']).to(device)
        output = model(images_dwi, images_T1).squeeze(1)
        # print(output)
...
def  train_valid_process_main(model, training_set, validation_set, batch_size):
    ...
    train_loader = DataLoader(training_set, batch_size=params['batch_size'], shuffle=True, num_workers=10)
...
if True: #model record
    params = {
        ...
        "img size": 384, 
        "img depth": 28,
        "adjust01": "valid size: 30%, class weight, Normalized[-1,1]",
        "augment": "RandRotated",
        "fixing": "Changed image channel to first, fix rotated setting"
        }
import monai
from monai.data import Dataset, DataLoader
from monai.transforms import  AddChanneld, LoadImaged, Compose, ToTensord, ScaleIntensityd, Rotate90d
from monai.transforms import  RandRotated, AsChannelFirstd
if True: #data augmentation, dataloader, 
    # print(nii_3t_train)
    KEYs= ('dwi', 't1')
    images = [{'dwi' : (os.path.join('.',dwi_dir, "{0}_dwi.nii.gz".format(i))), 't1' : (os.path.join('.', t1_dir, "{0}_T1.nii.gz".format(i))), 'target': t} for (i,t) in zip(nii_3t_train, table_label)]
    X_train, X_test, y_train, y_test = train_test_split(images, np.array(table_label), stratify=list(table_label), test_size=0.30, random_state=123) #seed = 42, 123
    # Transform edit
    training_transform = Compose([LoadImaged(KEYs,image_only=True, dtype=np.float32),
                                                                    AsChannelFirstd(KEYs),
                                                                    AddChanneld(KEYs),
                                                                    ScaleIntensityd(keys=['dwi', 't1'], maxv=1, minv=-1),
                                                                    Rotate90d(keys=['t1','dwi'], spatial_axes=(1,2),k=1),
                                                                    RandRotated(keys=['dwi','t1'], mode=('bilinear', 'bilinear'), range_x=np.pi/15, prob=0.5),
                                                                    ToTensord(KEYs),
                                                                    ])

    validation_transform = Compose([LoadImaged(KEYs,image_only=True, dtype=np.float32),
                                                                    AsChannelFirstd(KEYs),
                                                                    AddChanneld(KEYs),
                                                                    ScaleIntensityd(keys=['dwi', 't1'], maxv=1, minv=-1),
                                                                    Rotate90d(keys=['t1','dwi'], spatial_axes=(1,2),k=1),
    ...
    training_set = Dataset(data=X_train, transform=training_transform)
