...
def  train_valid_process_main(model, training_set, validation_set, batch_size):
    ...
    train_loader = DataLoader(training_set, batch_size=2, shuffle=True, num_workers=10)
...
from monai.transforms import  AddChanneld, LoadImaged, Compose, ToTensord, ScaleIntensityd
if True: #data augmentation, dataloader, 
    ...
    X_train, X_test, y_train, y_test = train_test_split(images, np.array(table_label), stratify=list(table_label), test_size=0.25, random_state=123) #seed = 42, 123
    # Transform edit
    ...
    project_folder = f"2022.02.07 - Test03"
